...
/*		volumes.push(context.createGainNode());
	...
	*/
	...
	/*	b = new Gibberish.Sequencer({
		...
		allBuffers.push(new Gibberish.Sampler({ file:daisyVals.buffer }).connect(prefxbus)); 
		allBuffers[buffNum].start = (daisyVals.sample*44100);
		allBuffers[buffNum].end = ((daisyVals.sample+6)*44100);
	...
	*/	
	...
	//	volumes[volumes.length-1].run();
		...
		allBuffers[samplersUsed%10].start = (daisyVals.sample*44100);
		allBuffers[samplersUsed%10].end = ((daisyVals.sample+6)*44100);
